...
std::vector<size_t> SimSearch::search_with_clusters(const float *query, size_t top_k){
    //donde guardaré las distancias del query a cada cluster
    std::vector<float> distancia_cluster;
    //para obtener la cantidad de clusters
    size_t k_clusters = mat_clusters.getK();
    ...
    */
    ...
    for(int i = 0; i< k_clusters; i++){
        const float* centroide_cluster = mat_clusters.getCentroid(i);
    ...
    //ordeno de distancias de query a cluster de menor a mayor:
    ...
    for(int i = 0; i<indices_cluster_ordenados.size(); i++){
        ...
        std::vector<size_t> puntos_del_cluster = mat_clusters.getInds(cluster_idx);
